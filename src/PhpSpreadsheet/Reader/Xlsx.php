...
class Xlsx extends BaseReader
{
     ...
     *
     * @param Worksheet $docSheet    [description]
     * @param string    $column      A, B, ... DX, ...
     * @param array     $columnAttrs array of attributes (indexes are attribute name, values are value)
     *                               'xfIndex', 'visible', 'collapsed', 'outlineLevel', 'width', ... ?
     */
    private function setColumnAttrs(Worksheet $docSheet, $column, array $columnAttrs)
    {
        if (isset($columnAttrs['xfIndex'])) {
            $docSheet->getColumnDimension($column)->setXfIndex($columnAttrs['xfIndex']);
        }
        if (isset($columnAttrs['visible'])) {
            $docSheet->getColumnDimension($column)->setVisible($columnAttrs['visible']);
        }
        if (isset($columnAttrs['collapsed'])) {
            $docSheet->getColumnDimension($column)->setCollapsed($columnAttrs['collapsed']);
        }
        if (isset($columnAttrs['outlineLevel'])) {
            $docSheet->getColumnDimension($column)->setOutlineLevel($columnAttrs['outlineLevel']);
        }
        if (isset($columnAttrs['width'])) {
            $docSheet->getColumnDimension($column)->setWidth($columnAttrs['width']);
        }
    }
     ...
     * @param array     $rowAttrs array of attributes (indexes are attribute name, values are value)
     *                               'xfIndex', 'visible', 'collapsed', 'outlineLevel', 'rowHeight', ... ?
     */
    private function setRowAttrs(Worksheet $docSheet, $row, array $rowAttrs)
    {
        if (isset($rowAttrs['xfIndex'])) {
            $docSheet->getRowDimension($row)->setXfIndex($rowAttrs['xfIndex']);
        }
        if (isset($rowAttrs['visible'])) {
            $docSheet->getRowDimension($row)->setVisible($rowAttrs['visible']);
        }
        if (isset($rowAttrs['collapsed'])) {
            $docSheet->getRowDimension($row)->setCollapsed($rowAttrs['collapsed']);
        }
        if (isset($rowAttrs['outlineLevel'])) {
            $docSheet->getRowDimension($row)->setOutlineLevel($rowAttrs['outlineLevel']);
        }
        if (isset($rowAttrs['rowHeight'])) {
            $docSheet->getRowDimension($row)->setRowHeight($rowAttrs['rowHeight']);
        }
    }
                                ...
                                foreach ($xmlSheet->cols->col as $col) {
                                    for ($i = (int) ($col['min']); $i <= (int) ($col['max']); ++$i) {
                                        if ($col['style'] && !$this->readDataOnly) {
                                            $columnsAttrs[Coordinate::stringFromColumnIndex($i)]['xfIndex'] = (int) $col['style'];
                                        }
                                        if (self::boolean($col['hidden'])) {
                                            $columnsAttrs[Coordinate::stringFromColumnIndex($i)]['visible'] = false;
                                        }
                                        if (self::boolean($col['collapsed'])) {
                                            $columnsAttrs[Coordinate::stringFromColumnIndex($i)]['collapsed'] = true;
                                        }
                                        if ($col['outlineLevel'] > 0) {
                                            $columnsAttrs[Coordinate::stringFromColumnIndex($i)]['outlineLevel'] = (int) $col['outlineLevel'];
                                        }
                                        $columnsAttrs[Coordinate::stringFromColumnIndex($i)]['width'] = (float) $col['width'];
                            ...
                            if ($xmlSheet && $xmlSheet->sheetData && $xmlSheet->sheetData->row) {
                                foreach ($xmlSheet->sheetData->row as $row) {
                                    if ($row['ht'] && !$this->readDataOnly) {
                                        $rowsAttrs[(int) $row['r']]['rowHeight'] = (float) $row['ht'];
                                    }
                                    if (self::boolean($row['hidden']) && !$this->readDataOnly) {
                                        $rowsAttrs[(int) $row['r']]['visible'] = false;
                                    }
                                    if (self::boolean($row['collapsed'])) {
                                        $rowsAttrs[(int) $row['r']]['collapsed'] = true;
                                    }
                                    if ($row['outlineLevel'] > 0) {
                                        $rowsAttrs[(int) $row['r']]['outlineLevel'] = (int) $row['outlineLevel'];
                                    }
                                    if ($row['s'] && !$this->readDataOnly) {
                                        $rowsAttrs[(int) $row['r']]['xfIndex'] = (int) $row['s'];
                                    }
                                }
                            }

                            // set columns/rows attrs
                            $columnsAttrsSet = [];
                            $rowsAttrsSet = [];
                            foreach ($columnsAttrs as $coordColumn => $columnAttrs) {
                                foreach ($rowsAttrs as $coordRow => $rowAttrs) {
                                    ...
                                    if (!isset($columnsAttrsSet[$coordColumn])) {
                                        $this->setColumnAttrs($docSheet, $coordColumn, $columnAttrs);
                                        $columnsAttrsSet[$coordColumn] = true;
                                    }
                                    if (!isset($rowsAttrsSet[$coordRow])) {
                                        $this->setRowAttrs($docSheet, $coordRow, $rowAttrs);
                                        $rowsAttrsSet[$coordRow] = true;
                                    }
                                }
                            }
