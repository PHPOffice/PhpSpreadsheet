...
class Bin2Oct
{
     ...
     * @param mixed $places
     */
    public static function funcBin2Oct($number, $places = null): string
    {
        ...
        if (strlen($number) == 10) {
            //    Two's Complement
            return str_repeat('7', 7) . substr(strtoupper(decoct((int) bindec(substr($number, -9)))), -3);
        }
        $octVal = (string) decoct((int) bindec($number));

        return Helpers::nbrConversionFormat($octVal, $places);
    }
}
