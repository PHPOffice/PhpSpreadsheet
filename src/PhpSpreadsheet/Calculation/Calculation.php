...
class Calculation
{
            ...
            if ($locale !== 'en_us') {
                ...
                foreach ($localeFunctions as $localeFunction) {
                    ...
                    if (str_contains($localeFunction, '=')) {
                        ...
                        if ((str_starts_with($fName, '*') || isset(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$fName])) && ($lfName != '') && ($fName != $lfName)) {
        ...
        while (true) {
            ...
            if ($opCharacter === '-' && !$expectingOperator) {                //    Is it a negation instead of a minus?
                ...
                if (is_array($d) && preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $d['value'], $matches)) {
                    ...
                    } elseif (isset(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$functionName])) {
                        $expectedArgumentCount = FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$functionName]['argumentCount'];
                ...
                if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $val, $matches)) {
                    ...
                    if (isset(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[strtoupper($matches[1])]) || isset(self::$controlFunctions[strtoupper($matches[1])])) {    // it's a function
    ...
    private function processTokenStack(mixed $tokens, ?string $cellID = null, ?Cell $cell = null)
    {
        ...
        foreach ($tokens as $tokenData) {
            ...
            if ($token instanceof Operands\StructuredReference) {
                ...
                if ((isset(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$functionName])) || (isset(self::$controlFunctions[$functionName]))) {    // function
                    $passByReference = false;
                    $passCellReference = false;
                    $functionCall = null;
                    if (isset(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$functionName])) {
                        $functionCall = FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$functionName]['functionCall'];
                        $passByReference = isset(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$functionName]['passByReference']);
                        $passCellReference = isset(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$functionName]['passCellReference']);
                    ...
                    for ($i = 0; $i < $argCount; ++$i) {
                        ...
                        if (
                            ($passByReference)
                            && (isset(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$functionName]['passByReference'][$a]))
                            && (FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$functionName]['passByReference'][$a])
                        ) {
    ...
    public function isImplemented(string $function): bool
    {
        ...
        $notImplemented = !isset(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$function]) || (is_array(FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$function]['functionCall']) && FunctionMapping::PHPSPREADSHEET_FUNCTIONS[$function]['functionCall'][1] === 'DUMMY');
    ...
    {
        return FunctionMapping::PHPSPREADSHEET_FUNCTIONS;
    }
    ...
    {
        $returnValue = [];
        foreach (FunctionMapping::PHPSPREADSHEET_FUNCTIONS as $functionName => $function) {
