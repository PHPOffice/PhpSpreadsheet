...
class Parser
{
    ...
    private function convert($token)
    {
        ...
        throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Unknown token $token");
    }
    ...
    private function convertString($string)
    {
        ...
        if (strlen($string) > 255) {
            throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException('String is too long');
        }
    ...
    private function convertRange2d($range, $class = 0)
    {
        ...
        if (preg_match('/^(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)\:(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)$/', $range)) {
            ...
            // TODO: use real error codes
            throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException('Unknown range separator');
        }
        ...
        if ($class == 0) {
            ...
            // TODO: use real error codes
            throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Unknown class $class");
        }
    ...
    private function packExtRef($ext_ref)
    {
        ...
        if (preg_match('/:/', $ext_ref)) {
            ...
            if ($sheet1 == -1) {
                throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Unknown sheet name $sheet_name1 in formula");
            }
            $sheet2 = $this->getSheetIndex($sheet_name2);
            if ($sheet2 == -1) {
                throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Unknown sheet name $sheet_name2 in formula");
            }
            ...
            if ($sheet1 == -1) {
                throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Unknown sheet name $ext_ref in formula");
            }
            $sheet2 = $sheet1;
        }
    ...
    private function cellToPackedRowcol($cell)
    {
        ...
        if ($col >= 256) {
            throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Column in: $cell greater than 255");
        }
        if ($row >= 65536) {
            throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Row in: $cell greater than 65536 ");
        }
    ...
    private function rangeToPackedRange($range)
    {
        ...
        if (($row1 >= 65536) or ($row2 >= 65536)) {
            throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Row in: $range greater than 65536 ");
        }
    ...
    private function fact()
    {
        if ($this->currentToken == '(') {
            ...
            if ($this->currentToken != ')') {
                throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("')' token expected.");
            }
            $this->advance(); // eat the ")"
            return $result;
        }
        ...
        throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException('Syntax error: '.$this->currentToken.', lookahead: '.$this->lookAhead.', current char: '.$this->currentCharacter);
    }
    ...
    private function func()
    {
        ...
        while ($this->currentToken != ')') {
            if ($num_args > 0) {
                if ($this->currentToken == ',' || $this->currentToken == ';') {
                    ...
                    throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Syntax error: comma expected in function $function, arg #{$num_args}");
                }
        ...
        if (!isset($this->functions[$function])) {
            throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Function $function() doesn't exist");
        }
        $args = $this->functions[$function][1];
        // If fixed number of args eg. TIME($i, $j, $k). Check that the number of args is valid.
        if (($args >= 0) and ($args != $num_args)) {
            throw new \PhpOffice\PhpSpreadsheet\Writer\WriteException("Incorrect number of arguments in function $function() ");
        }
