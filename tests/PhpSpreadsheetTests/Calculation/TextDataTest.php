...
class TextDataTest extends TestCase
{
    ...
    /**
     * @dataProvider providerASC
     *
     * @param mixed $expectedResult
     * @param string $formula
     */
    public function testASC($expectedResult, $formula)
    {
        $result = Calculation::calculateFormula($formula);
        self::assertEquals($expectedResult, $result);
    }

    public function providerASC()
    {
        return [
            ['ABC', '=ASC("ＡＢＣ")'],
            ['123', '=ASC("１２３")'],
            ['!@#', '=ASC("！＠＃")'],
            ['A1B2c3', '=ASC("Ａ1Ｂ2ｃ3")'],
            ['HelloWorld', '=ASC("HelloWorld")'],
            ['', '=ASC("")'],
            ['TRUE', '=ASC(TRUE)'],
            ['FALSE', '=ASC(FALSE)'],
            ['123.45', '=ASC(123.45)'],
            ['ｶﾀｶﾅ', '=ASC("カタカナ")'], // Full-width Katakana to half-width
            ['ひらがな', '=ASC("ひらがな")'], // Hiragana should remain Hiragana (as per 'a' option)
            ['ABCｶﾀｶﾅ123', '=ASC("ＡＢＣカタカナ１２３")'],
        ];
    }
}
